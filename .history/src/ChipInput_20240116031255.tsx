      ...
      <div className="flex flex-col w-[50vh]">
        <div className="flex flex-wrap">
          {chips.map((chip) => (
            <div
              key={chip.id}
              className="bg-[#037bfc] text-white rounded-md m-[5px] px-[5px] py-[12px] cursor-pointer flex items-center relative"
              onClick={() => handleChipRemove(chip.id)}
            >
              {chip.label}
              <span className="hover:border-white hover:bg-red-600 hover:border-2 hover:rounded-md px-1 mx-2 absolute right-60">X</span>
            </div>
          ))}
          <input
            ref={inputRef}
