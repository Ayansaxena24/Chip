      ...
      <div className="flex flex-col w-[50vh]">
        <div className="flex flex-wrap">
          {chips.map((chip) => (
            <div
              key={chip.id}
              className="bg-[#037bfc] text-white rounded-md m-[5px] px-[5px] py-[10px] cursor-pointer flex items-center"
              onClick={() => handleChipRemove(chip.id)}
            >
              {chip.label}
              <span className="hover:border-white hover:bg-red-600 hover:border-2 hover:rounded-md px-1 ml-2">X</span>
            </div>
          ))}
          <input
            ref={inputRef}
